...
export function quoteValue(quantity:string,quantityDecimals:number,quote:MarketQuote,targetDecimals:number):string {
 if(!/^(0|[1-9]\d{0,77})$/.test(quantity)||!Number.isInteger(quantityDecimals)||quantityDecimals<0||quantityDecimals>18||!Number.isInteger(targetDecimals)||targetDecimals<0||targetDecimals>18)throw new ProviderValidationError('Invalid valuation units.');
...
export function parseCoinGeckoQuote(text:string,now=Date.now()):MarketQuote {
 if(text.length>8192)throw new ProviderValidationError('Quote response too large.');
 ...
 if(members.length!==3||new Set(members).size!==3)throw new ProviderValidationError('Ambiguous quote response.');
 const token=text.match(/"usd"\s*:\s*(\d+(?:\.\d+)?(?:[eE][+-]?\d+)?)(?=\s*[,}])/);
 if(!token)throw new ProviderValidationError('Missing numeric USD quote.');
 ...
 if(priceDecimals<0){if(priceDecimals< -30)throw new ProviderValidationError('Price exceeds supported precision.');digits+='0'.repeat(-priceDecimals);priceDecimals=0;}
 const timestamp=parsed.zignaly.last_updated_at*1000;
 if(!Number.isSafeInteger(timestamp)||timestamp>now+60000)throw new ProviderValidationError('Invalid quote timestamp.');
...
/** Cancellation is best effort with a 250ms grace inside the provider's 11s permit.
 * Await normal cleanup; a broken cancel implementation cannot hang admission indefinitely. */
export async function cleanupMarketBody(cancel:()=>Promise<unknown>):Promise<void>{
 let timer:ReturnType<typeof setTimeout>|undefined;
 try{await Promise.race([Promise.resolve().then(cancel),new Promise<void>(resolve=>{timer=setTimeout(resolve,250);})]);}catch{/* Preserve the original sanitized result. */}finally{if(timer!==undefined)clearTimeout(timer);}
}
export async function boundedQuoteText(response:Response,limit=8192):Promise<string>{
 if(Number(response.headers.get('content-length'))>limit){await cleanupMarketBody(()=>response.body?.cancel()??Promise.resolve());throw new ProviderValidationError('Quote response too large.');}
 if(!response.body)throw new ProviderValidationError('Empty quote response.');
 const reader=response.body.getReader();let length=0;const chunks:Uint8Array[]=[];
 try{while(true){
 let chunk:ReadableStreamReadResult<Uint8Array>;
 try{chunk=await reader.read();}catch(error){throw new ProviderTransportError(error instanceof Error&&(error.name==='AbortError'||error.name==='TimeoutError')?'TIMEOUT':error instanceof Error&&error.name==='TypeError'?'NETWORK':'UNKNOWN');}
 if(chunk.done)break;length+=chunk.value.byteLength;
...
export function verifiedMarketQuote(raw:unknown,now=Date.now()):MarketQuote{
 ...
 if(quote.providerAssetId!==quote.marketRef.id||!['USD','EUR'].includes(quote.currency)||quote.source!==(quote.marketRef.kind==='rwa'?'CoinGecko tokenized RWA reference':'CoinGecko')||!quote.fetchedAt||(quote.marketRef.kind==='rwa'&&quote.currency!=='USD'))throw new ProviderValidationError('Unsupported market evidence.');
 for(const stamp of [quote.observedAt,quote.fetchedAt])if(stamp&&Date.parse(stamp)>now+60000)throw new ProviderValidationError('Future market evidence.');return quote;
}
const object=(raw:unknown)=>{if(!raw||typeof raw!=='object'||Array.isArray(raw)||raw instanceof JsonNumber)throw new ProviderValidationError('Invalid quote object.');return raw as Record<string,unknown>;};
...
export function parseCoinTokenQuote(text:string,request:MarketQuoteRequest,contractAddress:string,now=Date.now()):MarketQuote{
 ...
 if(ref.marketRef.kind!=='coin')throw new ProviderValidationError('Token-address fallback supports coins only.');
 ...
 if(matches.length!==1||Object.keys(data).length!==1)throw new ProviderValidationError('Unexpected token-price identity.');
 const row=object(matches[0]![1]);
 const stamp=row.last_updated_at;
 if(!(stamp instanceof JsonNumber)||!/^[0-9]+$/.test(stamp.lexeme))throw new ProviderValidationError('Missing provider observation time.');
