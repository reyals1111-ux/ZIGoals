...
const delegation=z.object({delegation:z.object({delegator_address:publicZigAddress,validator_address:validatorAddress}),balance:coin});
...
export async function readNativePositions(mode:ReadMode,account:string,fetcher:typeof fetch=fetch):Promise<Position[]>{
 ...
 const latest=z.object({block:z.object({header:z.object({height:units.refine(v=>BigInt(v)>0n),chain_id:z.string(),time:z.iso.datetime({offset:true})})})}).parse(await readAt('/cosmos/base/tendermint/v1beta1/blocks/latest')).block.header;
 if(latest.chain_id!==network.chainId)throw Error('Public block network does not match.');
 const height=latest.height;
 const read=(path:string)=>readAt(path,height);
...
export {replaceObservation,markObservationError} from './positions';
