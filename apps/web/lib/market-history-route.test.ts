...
it('rejects private payloads, query overrides and excessive request bodies before any provider access',async()=>{
 ...
 for(const body of [{request,quantity:'private'},{request:{...request,wallet:'private'}},{request:{...request,marketRef:{...request.marketRef,notes:'private'}}},{request:{...request,range:'max'}},{request,localPoints:[{at:'private'}]}])expect((await POST(new Request('https://local/api/market-history',{headers:{"Content-Type":"application/json"},method:'POST',body:JSON.stringify(body)}))).status).toBe(400);
 expect((await POST(new Request('https://local/api/market-history?url=https://evil',{headers:{"Content-Type":"application/json"},method:'POST',body:JSON.stringify({request})}))).status).toBe(400);
 expect((await POST(new Request('https://local/api/market-history',{headers:{"Content-Type":"application/json"},method:'POST',body:'x'.repeat(8193)}))).status).toBe(400);expect(fetcher).not.toHaveBeenCalled();
});
it('returns a clean no-key fallback without anonymous provider requests or private request credentials',async()=>{
 ...
 const result=await POST(new Request('https://local/api/market-history',{method:'POST',body:JSON.stringify({request}),headers:{"Content-Type":"application/json",Cookie:'private',Authorization:'private'}}));expect(result.status).toBe(503);expect((await result.json()).history).toBeNull();expect(fetcher).not.toHaveBeenCalled();
});
it('retains exact history on provider failure and admits only public headers upstream',async()=>{
 ...
 const fetcher=vi.fn(async()=>{if(fail)return new Response('private upstream details',{headers:{"Content-Type":"application/json"},status:500});return new Response(`{"prices":[[${now-3600000},123.123456789123456789],[${now},124]],"market_caps":[],"total_volumes":[]}`, {headers:{"Content-Type":"application/json"}});});vi.stubGlobal('fetch',fetcher);const {POST}=await import('../app/api/market-history/route');
 const incoming=()=>new Request('https://local/api/market-history',{method:'POST',headers:{"Content-Type":"application/json",Cookie:'private',Authorization:'private'},body:JSON.stringify({request})});const first=await (await POST(incoming())).json();expect(first.history.points[0].value).toBe('123123456789123456789');expect(JSON.stringify(fetcher.mock.calls)).not.toContain('private');
