...
it('provider insights use batched currencies, shared admission and return partial success on quota failure',async()=>{
 const urls:string[]=[],headers:unknown[]=[];const fetcher:typeof fetch=async(input,init)=>{urls.push(String(input));headers.push(init?.headers);return new Response(String(input).includes('/rwas/')?'quota secret detail':payload,{status:String(input).includes('/rwas/')?429:200});};
...
it('splits provider batches at 250 IDs per currency without a request per asset',async()=>{
 const seen:Array<{ids:number;currency:string|null}>=[];const provider=createCoinGeckoProvider({key:()=> 'fixture-key',clock:()=>now,fetcher:async input=>{const url=new URL(String(input));seen.push({ids:url.searchParams.get('ids')!.split(',').length,currency:url.searchParams.get('vs_currency')});return new Response('[]');}});
