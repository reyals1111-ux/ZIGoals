...
export function createMarketQuoteCache(load:(requests:readonly MarketQuoteRequest[],force?:boolean)=>Promise<MarketQuoteLoadResult>,clock=()=>Date.now()){
 let state:QuoteSnapshot={quotes:[],now:clock(),loading:false,error:null};const attempts=new Map<string,number>(),listeners=new Set<()=>void>();let pending:Promise<void>|null=null;let storage:Pick<Storage,'getItem'|'setItem'>|undefined;
 ...
 const requests=uniqueMarketRequests(raw);if(!requests.length)return;if(pending){await pending;return refresh(requests,force);}
 ...
 pending=(async()=>{try{const loaded=await load(needed,force);const result='quotes' in loaded?loaded:{quotes:loaded,error:null};const quotes=result.quotes.map(q=>verifiedMarketQuote(q,clock()));if((quotes.length!==needed.length&&!result.error)||quotes.some(q=>!needed.some(r=>marketRequestKey(r)===key(q)))||new Set(quotes.map(key)).size!==quotes.length)throw Error('Incomplete market response.');
 ...
 }catch{state={...state,error:'Market prices could not be refreshed. Last verified evidence is retained; manual valuation remains available.'};}finally{state={...state,loading:false,now:clock()};pending=null;emit();}})();await pending;
 }
 return {getSnapshot:()=>state,subscribe:(listener:()=>void)=>{listeners.add(listener);return()=>{listeners.delete(listener);};},hydrate,tick,refresh};
}
