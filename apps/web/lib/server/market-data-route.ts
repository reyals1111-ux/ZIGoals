...
export async function configuredDurableCatalog(signal?:AbortSignal){
 ...
 try{const response=await runtime.binding.fetch(new Request('https://market.internal/catalog',{method:'POST',signal,headers:{'content-type':'application/json'},body:'{"version":1}'}));if(!response.ok)return unavailable;return catalogSchema.parse(JSON.parse(await boundedQuoteText(response,32*1024*1024)));}catch{return unavailable;}
}
export async function configuredDurableHistory(request:MarketHistoryRequest,signal?:AbortSignal){
 const runtime=await marketRuntime();if(runtime.mode==='development')return null;
 const unavailable={history:null,error:HISTORY_UNAVAILABLE,stale:true,nextAttemptAt:Date.now()+60000};
 if(runtime.mode!=='durable')return unavailable;
 try{const response=await runtime.binding.fetch(new Request('https://market.internal/history',{method:'POST',signal,headers:{'content-type':'application/json'},body:JSON.stringify({version:1,request})}));if(!response.ok)return unavailable;const raw=JSON.parse(await boundedQuoteText(response,2*1024*1024));if(!raw.history)return unavailable;const history=verifiedMarketHistory(raw.history,request);return {history,error:raw.error?HISTORY_UNAVAILABLE:null,stale:historyIsStale(history),nextAttemptAt:Date.now()+60000};}catch{return unavailable;}
}
export async function configuredDurableInsights(requests:readonly MarketQuoteRequest[],signal?:AbortSignal){
 const runtime=await marketRuntime();if(runtime.mode==='development')return null;
 ...
 return fetchPublicMarketInsights(requests,false,async(_input,init)=>{
  ...
  return runtime.binding.fetch(new Request('https://market.internal/insights',{method:'POST',signal,headers:{'content-type':'application/json'},body:JSON.stringify({version:1,requests:body.requests})}));
 });
}
