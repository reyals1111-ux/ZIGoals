...
export const config={policy,month:{id:'fixture',start:now-1000,end:now+100000},quoteCost:3,operationCosts:{token:6},leaseMs:10000,maxAttempts:16,maxWorks:16};
class Memory implements AtomicMarketStorage{rows=new Map<string,unknown>();async get<T>(k:string){return structuredClone(this.rows.get(k)) as T|undefined;}async put(k:string,v:unknown){this.rows.set(k,structuredClone(v));}async delete(key:string){this.rows.delete(key);}async transaction<T>(f:(s:AtomicMarketStorage)=>Promise<T>):Promise<T>{return f(this);}}
...
test('missing credential, denial and unsupported RWA never dispatch provider I/O',async()=>{
 ...
 const unsupported=await dispatchDurableQuotes([{marketRef:{provider:'coingecko',kind:'rwa',id:'gold',assetType:'commodity'},currency:'EUR'}],{command,key:'fixture',fetcher,clock:()=>now});expect(unsupported.results[0]?.failure).toBe('UNSUPPORTED');expect(fetcher).not.toHaveBeenCalled();
});
