...
export async function configuredDurableQuotes(requests:readonly MarketQuoteRequest[],loadEnv?:()=>Promise<QuoteEnvironment>,signal?:AbortSignal,cancelToken?:string){
 ...
 try{const response=await runtime.binding.fetch(new Request('https://market.internal/quotes',{method:'POST',signal,headers:{'content-type':'application/json',...(cancelToken?{'x-market-cancel-token':cancelToken}:{})},body:JSON.stringify({version:1,requests})}));if(!response.ok)return unavailable();return parseMarketPairWire(JSON.parse(await boundedQuoteText(response,1024*1024)),requests);}catch{return unavailable();}
}
