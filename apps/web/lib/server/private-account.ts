...
export async function privateAccountRequest(request:Request,config:AccountConfig|null,fetcher:typeof fetch=fetch):Promise<Response>{
 const parsed=configSchema.safeParse(config);if(!parsed.success)return reply({error:'HOSTED_CONFIGURATION_REQUIRED',message:'Email and encrypted sync have not been configured. Local records remain available.'},503);
 const cfg=parsed.data,origin=new URL(request.url).origin;
 ...
 try{
  if(request.method==='GET'){
   ...
   const remote=await upstream(`${cfg.syncOrigin}/v1/vault${cursor?'?cursor='+encodeURIComponent(cursor):''}`,{headers:{authorization:`Bearer ${token}`,origin}});
  ...
  const action=actionSchema.parse(await readBounded(request,1_010_000));
  if(action.action==='signout'){
   let remoteRevocationConfirmed=!token;
   if(token){try{const result=await upstream(`${cfg.authOrigin}/auth/v1/logout?scope=local`,{method:'POST',headers:{apikey:cfg.publicKey,authorization:`Bearer ${token}`}});remoteRevocationConfirmed=result.ok;await result.body?.cancel().catch(()=>{});}catch{remoteRevocationConfirmed=false;}}
   return reply({signedOut:true,remoteRevocationConfirmed},200,{'Set-Cookie':cookie('',0)});
  }
  if(action.action==='sync'){
   if(!token)return reply({error:'SIGN_IN_REQUIRED'},401);
   const remote=await upstream(`${cfg.syncOrigin}/v1/vault`,{method:'POST',headers:{authorization:`Bearer ${token}`,origin,'content-type':'application/json'},body:JSON.stringify(action.operation)});
  ...
  const session=z.object({access_token:z.string().regex(/^[A-Za-z0-9._-]{1,4096}$/),expires_in:z.number().int().min(1).max(86400)}).passthrough().parse(await readBounded(remote,32768));
  return reply({signedIn:true},200,{'Set-Cookie':cookie(session.access_token,session.expires_in)});
