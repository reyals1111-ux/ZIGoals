...
export async function admitMarketBreakers(tx:AtomicMarketStorage,policy:BreakerPolicy|undefined,id:string,endpoint:string,now:number):Promise<StoredPermit[]|null>{
 if(!policy)return [];
 const scopes:BreakerScope[]=[{kind:'account-authentication'},{kind:'account-throttle'},{kind:'endpoint-availability',endpoint},{kind:'endpoint-integrity',endpoint}];
 const updates=[];
...
export async function settleMarketBreakers(tx:AtomicMarketStorage,policy:BreakerPolicy|undefined,permits:StoredPermit[]|undefined,outcome:'VERIFIED'|ProviderFailureCategory,now:number,{endpoint,accountThrottle}:{endpoint?:string;accountThrottle?:{distinctEndpoints:number;windowMs:number}}={}){
 ...
 for(const {key,scope,permit}of permits){const state=await tx.get<BreakerState>(key);if(!state)continue;
  const relevant=outcome==='VERIFIED'||scope.kind==='account-throttle'&&outcome==='THROTTLED'&&correlated||scope.kind==='account-authentication'&&outcome==='AUTHENTICATION'||scope.kind==='endpoint-integrity'&&outcome==='MALFORMED'||scope.kind==='endpoint-availability'&&['THROTTLED','UPSTREAM_5XX','NETWORK','TIMEOUT'].includes(outcome);
