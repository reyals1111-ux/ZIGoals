 ...
 for(const group of [owners.filter(o=>o.request.marketRef.kind==='coin'&&o.request.marketRef.id!=='zignaly'),owners.filter(o=>o.request.marketRef.kind==='coin'&&o.request.marketRef.id==='zignaly'),owners.filter(o=>o.request.marketRef.kind==='rwa')]){
  ...
  try{
   ...
   try{
    ...
    const text=await boundedQuoteText(response,1024*1024);result=parseProviderEvidence(()=>rwa?parseRwaQuotes(text,members,clock()):parseCoinQuotes(text,members,clock()));
   }catch(error){
    ...
    await chargedMarketRead(fallbackUrl,'token',64*1024,{command,key,fetcher,signal,validate:text=>{result=members.map(member=>parseCoinTokenQuote(text,member,ZIG_CONTRACT,clock()));},fallback:{parentId:operation!,associations:group.map(({work,lease})=>({work,lease})),onAttempt:id=>{operation=id;}}});
   }
