...
it('retained presentation events and archived-period history survive later edits',async()=>{const {restoreAsset}=await import('./asset-management');const p=cash('100'),q={...cash('100'),id:'other'};let s=captureValuations({...base(),positions:[p,q]},[],now);s=archiveAsset(s,p.id,false,now+1000);s=captureValuations({...s,positions:s.positions.map(p=>p.id==='other'?{...p,valuation:{...p.valuation!,value:'15000',observedAt:new Date(now+86400000).toISOString()}}:p)},[],now+86400000);s=restoreAsset(s,p.id,now+86400000+1000);const before=wealthHistory(s);for(let i=0;i<241;i++)s=saveAsset(s,{...s.positions[1]!,notes:String(i),observedAt:new Date(now+86400000+2000+i).toISOString()});expect(s.assetEvents.some(e=>e.kind==='archived')).toBe(true);expect(wealthHistory(s)).toEqual(before);expect(before[0]!.points.at(-1)!.value).toBe('15000');});
it('currently archived membership survives 241 edits and a backup round trip',()=>{
 ...
 expect(s.assetEvents.some(e=>e.positionId===p.id)).toBe(true);
