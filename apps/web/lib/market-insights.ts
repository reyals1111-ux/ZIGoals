...
export function verifiedMarketInsight(raw:unknown,request:MarketQuoteRequest,now=Date.now()):MarketInsight{
 ...
 if(marketRequestKey(entry)!==marketRequestKey(request)||(!coin&&request.marketRef.kind==='rwa'&&entry.marketRef.kind==='rwa'&&entry.marketRef.assetType!==request.marketRef.assetType)||entry.marketBasis!==(coin?'coin':'tokenized')||entry.source!==(coin?'CoinGecko':'CoinGecko tokenized RWA reference'))throw new ProviderValidationError('Invalid insight identity.');
 const fetched=Date.parse(entry.fetchedAt);if(fetched<=0||fetched>now+60000||(entry.observedAt&&(Date.parse(entry.observedAt)<=0||Date.parse(entry.observedAt)>fetched+60000)))throw new ProviderValidationError('Invalid insight timestamp.');
 if((entry.change24h!==null||entry.sparkline)&&!entry.observedAt)throw new ProviderValidationError('Movement requires provider time.');
 if(entry.sparkline?.fetchedAt&&entry.sparkline.fetchedAt!==entry.fetchedAt)throw new ProviderValidationError('Invalid sequence timestamp.');
 if(!coin&&entry.currency!=='USD'&&(entry.change24h!==null||entry.sparkline||entry.observedAt))throw new ProviderValidationError('RWA movement is USD only.');
 return entry;
}
function object(raw:unknown):Record<string,unknown>{if(!raw||typeof raw!=='object'||Array.isArray(raw)||raw instanceof JsonNumber)throw new ProviderValidationError('Invalid insights response.');return raw as Record<string,unknown>;}
...
export function parseMarketInsights(text:string,raw:readonly MarketQuoteRequest[],now=Date.now()):MarketInsight[]{
 ...
 if(!Array.isArray(parsed)||parsed.length>250)throw new ProviderValidationError('Invalid insight rows.');const seen=new Set<string>();const entries:MarketInsight[]=[];
 for(const rawRow of parsed){const row=object(rawRow);if(typeof row.id!=='string'||seen.has(row.id))throw new ProviderValidationError('Invalid or duplicate insight identity.');seen.add(row.id);const members=requests.filter(r=>r.marketRef.id===row.id);if(!members.length)throw new ProviderValidationError('Unexpected insight identity.');
 for(const request of members){const coin=request.marketRef.kind==='coin';if(!coin&&request.marketRef.kind==='rwa'&&row.asset_type!==request.marketRef.assetType)throw new ProviderValidationError('Wrong RWA asset type.');
 ...
 if(observedAt){const change=data.price_change_percentage_24h_in_currency??data.price_change_percentage_24h;if(change!=null){if(!(change instanceof JsonNumber))throw new ProviderValidationError('Invalid movement.');change24h=exactChange.parse(change.lexeme);}
 if(data.sparkline_in_7d!=null){const prices=object(data.sparkline_in_7d).price;if(!Array.isArray(prices)||prices.length>MAX_SPARKLINE_PRICES)throw new ProviderValidationError('Invalid sequence.');const values=prices.map(price=>{const exact=decimalLexeme(price);return {value:exact.price,decimals:exact.priceDecimals};});if(values.length>=2)sparkline={range:'7d',timestamps:'unavailable',fetchedAt:new Date(now).toISOString(),prices:values};}}
