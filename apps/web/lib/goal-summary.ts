...
export function privateGoalSummary(data:Platform,g:PrivateGoal,quotes:readonly MarketQuote[]=[],now=Date.now()):GoalSummary{
 ...
 try{if(g.type!=='PROJECT'){const pulse=fundingHealth(data,g.id,now,quotes);funding=pulse.status.replaceAll('_',' ');if(g.plan?.active&&g.status!=='closed')nextContributionDate=pulse.nextDate;}}catch{funding='Review assumptions';}
