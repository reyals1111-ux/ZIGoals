...
export function manualSourcePosition(input:ManualSource,id=crypto.randomUUID(),observedAt=new Date().toISOString()){
 ...
 const asset=input.category==='Cash'?currency:input.category==='Precious metals'?`${input.metal||'Gold'} ${input.unit||'grams'}`:(input.symbol||input.name).trim().toUpperCase();
