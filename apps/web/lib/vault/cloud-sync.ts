...
export type SyncState={version:1;base:PrivateData;revision:number;headRevision:number;headDigest:string|null;pending:CloudOperation|null;pendingHealth?:boolean;pendingPolicy?:number};
export const syncStateSchema=z.object({version:z.literal(1),base:z.partialRecord(z.enum(DOMAINS),z.string().max(32_000_000)),revision:z.number().int().nonnegative(),headRevision:z.number().int().nonnegative(),headDigest:z.string().regex(/^[a-f0-9]{64}$/).nullable(),pendingHealth:z.boolean().optional(),pendingPolicy:z.number().int().positive().optional(),pending:z.object({protocol:z.literal(1),vault:z.uuid(),operation:z.uuid(),base:z.number().int().nonnegative(),changes:z.array(rowSchema).max(100),manifest:manifestSchema.optional()}).strict().nullable()}).strict();
