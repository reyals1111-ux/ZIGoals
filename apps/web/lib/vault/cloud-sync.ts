...
export type SyncState={version:1;epoch?:number;base:PrivateData;revision:number;headRevision:number;headDigest:string|null;pending:CloudOperation|null;pendingHealth?:boolean;pendingPolicy?:number};
export const syncStateSchema=z.object({version:z.literal(1),epoch:epochSchema.optional(),base:z.partialRecord(z.enum(DOMAINS),z.string().max(32_000_000)),revision:z.number().int().nonnegative(),headRevision:z.number().int().nonnegative(),headDigest:z.string().regex(/^[a-f0-9]{64}$/).nullable(),pendingHealth:z.boolean().optional(),pendingPolicy:z.number().int().positive().optional(),pending:z.object({protocol:z.literal(1),vault:z.uuid(),operation:z.uuid(),base:z.number().int().nonnegative(),changes:z.array(rowSchema).max(100),manifest:manifestSchema.optional()}).strict().nullable()}).strict();
...
export async function synchronize(transport:CloudTransport,journal:Journal,key:CryptoKey,manifest:VaultManifest,local:PrivateData,validate:(data:PrivateData,prior?:PrivateData)=>void,fence:()=>void,allowed:readonly Domain[]=DOMAINS){
 ...
 fence();return {data:next,revision,commit:async()=>{fence();await journal.write({version:1,epoch:manifest.epoch,base:{...state.base,...next},revision,headRevision,headDigest,pending:null});}};
}
export class SyncJournal implements Journal{
 ...
 async read():Promise<SyncState>{const db=await this.database();try{return await new Promise((resolve,reject)=>{const r=db.transaction('state').objectStore('state').get(this.account);r.onsuccess=()=>{const value=r.result;if(value&&value.version!==1){reject(Error('Newer sync journal requires an app update.'));return;}try{resolve(value?syncStateSchema.parse(value):{version:1,base:{},revision:0,headRevision:0,headDigest:null,pending:null});}catch{reject(Error('Sync journal is damaged. Export before recovery.'));}};r.onerror=()=>reject(Error('Sync journal unreadable.'));});}finally{db.close();}}
