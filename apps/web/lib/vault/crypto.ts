...
const uuid=z.uuid();
export const recordContextSchema=z.object({vault:uuid,domain:z.enum(['finance','habits','health','settings']),object:uuid,revision:z.number().int().min(1).max(Number.MAX_SAFE_INTEGER),epoch:z.literal(1)}).strict();
...
export const manifestSchema=z.object({version:z.literal(1),vault:uuid,epoch:z.literal(1),wrapped:envelopeSchema}).strict();
...
async function domainKey(root:CryptoKey,vault:string,domain:string){return crypto.subtle.deriveKey({name:'HKDF',hash:'SHA-256',salt:bytes(vault),info:bytes(`zigoals:vault:v1:${domain}:epoch1`)},root,{name:'AES-GCM',length:256},false,['encrypt','decrypt']);}
...
export async function createVault(vault=crypto.randomUUID()):Promise<{key:CryptoKey;recovery:string;manifest:VaultManifest}>{
 uuid.parse(vault);const raw=crypto.getRandomValues(new Uint8Array(32)),recovery=encode(crypto.getRandomValues(new Uint8Array(32)));
 try{const wrapped=await encrypt(await wrappingKey(recovery),raw,`zigoals:root:v1:${vault}:epoch1`);return {key:await rootKey(raw),recovery,manifest:{version:1,vault,epoch:1,wrapped}};}finally{raw.fill(0);}
}
export async function unlockVault(input:unknown,recovery:string):Promise<CryptoKey>{
 const manifest=manifestSchema.parse(input);const raw=await decrypt(await wrappingKey(recovery),manifest.wrapped,`zigoals:root:v1:${manifest.vault}:epoch1`);
...
export async function sealRecord(key:CryptoKey,context:RecordContext,value:unknown):Promise<EncryptedEnvelope>{
 const aad=contextText(context);return encrypt(await domainKey(key,context.vault,context.domain),bytes(JSON.stringify(value)),aad);
}
export async function openRecord(key:CryptoKey,context:RecordContext,envelope:unknown):Promise<unknown>{
 const aad=contextText(context),raw=await decrypt(await domainKey(key,context.vault,context.domain),envelope,aad);
