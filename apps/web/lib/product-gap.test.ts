...
it('never presents an entire manual holding valuation as an unavailable unit reference price',()=>{
 const element=html(createElement(AssetDetail,{id:'showcase-aapl'}));expect(element.querySelector('.asset-detail-price')?.textContent).toBe('Unavailable');expect(element.querySelector('[aria-label="Your holding"]')?.textContent).toContain('$7500');
});
...
it('Goal cards show remaining, target date, cadence and the next real scheduled date together',()=>{
 const goal=env.data.goals.find(g=>g.id==='9201')!;goal.plan={amount:'50000',asset:'USD',decimals:2,cadence:'monthly',nextDate:'2026-09-23',endDate:'2027-09-23',active:true};const summary=privateGoalSummary(env.data,goal,[],env.now);const element=html(createElement(GoalSummaryCard,{summary}));expect(element.textContent).toContain('$11000 remaining');expect(element.textContent).toContain('Target 2027-09-21');expect(element.textContent).toContain('Next contribution');expect(element.textContent).toContain('2026-09-23');expect(element.textContent).toContain('$500 / monthly');
});
