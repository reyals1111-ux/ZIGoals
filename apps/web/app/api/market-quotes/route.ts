...
export async function GET(request:Request):Promise<Response>{
 ...
 const durable=await configuredDurableQuotes([nativeZigRequest],undefined,request.signal);if(durable)return Response.json(durable,{status:durable.quotes.length?200:503,headers});
...
export async function POST(request:Request):Promise<Response>{
 ...
 try{if(new URL(request.url).search)throw Error('Query');const body=bodySchema.parse(JSON.parse(await boundedQuoteText(new Response(request.body),128*1024)));const requests=uniqueMarketRequests(body.requests);const durable=await configuredDurableQuotes(requests,undefined,request.signal);if(durable)return Response.json(durable,{status:durable.quotes.length||!requests.length?200:503,headers});await serverMarketCache.refresh(requests,body.refresh);const result=requestedEvidence(requests);return Response.json({...result,error:!result.quotes.length&&requests.length&&!process.env.COINGECKO_DEMO_API_KEY?.trim()?setupError:result.error},{status:result.quotes.length||!requests.length?200:503,headers});
