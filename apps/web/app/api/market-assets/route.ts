...
export async function GET(request:Request):Promise<Response>{
 ...
 const result=(await configuredDurableCatalog(request.signal))??await coinGeckoProvider.catalog();return Response.json(result,{status:result.assets.length?200:503,headers});
}
