...
export default function ActivityPage() {
  ...
  return (
    <div className="activity-page">
      ...
      <section className="panel activity-timeline" aria-label="Unified private activity"><div className="activity-filter-row"><div><p className="eyebrow">YOUR PROGRESS, IN MOMENTS</p><h2>One journey. Every step.</h2></div><nav className="view-tabs" aria-label="Activity categories">{["ALL", "GOAL", "HABIT", "HEALTH"].map(value => <button key={value} aria-pressed={category === value} onClick={() => { setCategory(value); setLimit(30); }}>{value === "ALL" ? "All" : value[0] + value.slice(1).toLowerCase()}</button>)}</nav></div><ActivityFeed limit={limit} category={category}/><button className="quiet" onClick={() => setLimit(value => value + 30)}>Show more activity</button><p className="fine">Habit and Health entries reflect current saved logs. Corrections update this view; removing a log removes it here. This is private browser history, separate from testnet receipts.</p></section>
      <p className="eyebrow">
        {s.historySource} ·{" "}
        {s.mode === "local"
