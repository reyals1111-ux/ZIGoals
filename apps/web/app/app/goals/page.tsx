...
export default function GoalsPage() {
  ...
  return <div className="goals-page dashboard">
    ...
    <PlatformNav/><TrackedGoals compact/>
    ...
    {!!s.goals.length && <div className="goals-toolbar"><nav className="tab-row view-tabs" aria-label="Goal views">{["Active", "All", "Closed"].map(view => <button key={view} aria-pressed={filter === view} onClick={() => setFilter(view)}>{view}</button>)}</nav><span>{visible.length} {visible.length === 1 ? "destination" : "destinations"}</span></div>}
