...
test('manual sources stay selected and all allocations are created',async({page})=>{
 await seed(page);await page.goto('/app/goals/new');await page.getByLabel('Goal name',{exact:true}).fill('All my sources');await page.getByRole('combobox',{name:'Goal type',exact:true}).selectOption('VALUE');await page.getByLabel('Target amount',{exact:true}).fill('100000');await page.getByRole('button',{name:'Continue'}).click();
 ...
 await page.getByRole('button',{name:category!,exact:true}).click();await page.getByLabel('Asset name',{exact:true}).fill(name!);if(symbol)await page.getByLabel('Symbol / ticker').fill(symbol);await page.getByLabel(category==='Cash'?'Cash amount':category==='Precious metals'?'Weight':'Quantity',{exact:true}).fill(quantity!);if(value)await page.getByLabel('Manual total value (optional)',{exact:true}).fill(value);if(category==='Stablecoins')await shot(page,'06-stablecoin-source');await page.getByRole('button',{name:'Save manual source',exact:true}).click();await expect(page.getByLabel('Wealth included in this Goal')).toContainText(name!);
 }
 ...
 await page.getByRole('button',{name:'Property',exact:true}).click();await page.getByLabel('Property name').fill('Example home');await page.getByLabel('Manual total value',{exact:true}).fill('350000');await shot(page,'05-property-source');await page.getByRole('button',{name:'Save manual source',exact:true}).click();await page.getByRole('button',{name:'Remove Example home',exact:true}).click();
 await page.getByRole('button',{name:'Continue'}).click();await page.getByRole('button',{name:'Continue'}).click();await expect(page.locator('.wizard')).toContainText('80000 USD');await page.getByRole('button',{name:'Create goal',exact:true}).click();await expect(page.getByTestId('tracked-progress')).toContainText('$80000');await expect(page.getByLabel('Asset mix')).toContainText('25.00%');expect(await page.locator('.goal-detail-progress .ring-value').count()).toBe(4);const saved=await page.evaluate(()=>JSON.parse(localStorage.getItem('zigoals:platform:v1')!));expect(saved.allocations.filter((a:{goalId:string})=>a.goalId===saved.goals.at(-1).id)).toHaveLength(4);
});
test('Wealth shows every category, preserves allocations and fits all viewports',async({page})=>{
 await mixed(page);await page.setViewportSize({width:1440,height:1100});await page.goto('/app/goals/tracked/91');await shot(page,'01-segmented-value-goal');await expect(page.getByRole('progressbar')).toHaveAttribute('aria-valuenow','80');await page.goto('/app/wealth');await expect(page.locator('.wealth-category')).toHaveCount(7);await expect(page.locator('.wealth-hero')).toContainText('$433500');await expect(page.locator('.wealth-hero')).toContainText('$80000');await shot(page,'03-wealth-desktop');await shot(page,'04-all-seven-categories');
