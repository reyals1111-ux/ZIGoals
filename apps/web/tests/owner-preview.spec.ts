...
test('owner setup: stake allocation, contribution plan and supporting Habit stay private',async({page},info)=>{
 ...
 await expect(page.getByTestId('tracked-progress').getByRole('progressbar')).toHaveAttribute('aria-valuenow','87.85');
...
test('APR persistence, account isolation, utility order and navigation',async({page})=>{
 await page.goto('/app');await page.getByRole('navigation',{name:'Main navigation'}).getByRole('link',{name:'Stake / Positions',exact:true}).click();const card=page.locator('.positions-scenario');await expect(card).toContainText('Not set');
...
test('responsive owner preview captures',async({page},info)=>{
 ...
 for(const width of info.project.name==='desktop'?[1440,768]:[390,320]){
  ...
  for(const [name,route] of [['today','/app'],['tracked-goals','/app/goals/tracked'],['positions','/app/goals/positions'],['goal-detail','/app/goals/tracked/81']]){
   ...
   if(process.env.RUN81_CAPTURE==='1'){await page.screenshot({path:info.outputPath(`${name}-${width}.png`),fullPage:true,animations:'disabled',scale:'css'});if(name==='today')await page.locator('.dashboard-summary').screenshot({path:info.outputPath(`today-summary-${width}.png`),animations:'disabled',scale:'css'});}
  }
 }
});
