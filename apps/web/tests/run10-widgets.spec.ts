...
test('Health-only setup persists and customization saves, hides, reorders and removes configuration',async({page},info)=>{
 ...
 const card=page.getByRole('article',{name:'My nourishment',exact:true});await expect(card).toBeVisible();const placed=page.locator('.dashboard-layout-grid > .placed-module').filter({has:card});const before=await placed.evaluate(el=>Array.from(el.parentElement!.children).indexOf(el));await card.getByRole('button',{name:'Move My nourishment up',exact:true}).click();
 await expect.poll(()=>placed.evaluate(el=>Array.from(el.parentElement!.children).indexOf(el))).toBe(before-1);await expect(placed).toBeFocused();
 ...
 await page.getByRole('button',{name:'Customize Today',exact:true}).click();await card.getByRole('button',{name:'Show',exact:true}).click();await card.getByRole('button',{name:'Edit',exact:true}).click();await page.getByRole('dialog',{name:'Edit widget'}).getByLabel('Title (optional)').fill('Meals at a glance');await page.getByRole('button',{name:'Save widget',exact:true}).click();await expect(page.getByRole('dialog',{name:'Edit widget'})).not.toBeVisible();
...
test('all four presets preview before applying; restored life card precedes attention; Escape preserves settings',async({page},info)=>{
 ...
 for(const width of [320,360,390,430,768,1440]){await page.setViewportSize({width,height:900});await expect.poll(()=>page.evaluate(()=>({width:innerWidth,scroll:document.documentElement.scrollWidth,overflow:[...document.querySelectorAll('main *')].filter(e=>!(e instanceof SVGElement)&&e.getBoundingClientRect().right>innerWidth+1).map(e=>({class:e.className,parent:e.parentElement?.className,text:e.textContent?.slice(0,80),right:e.getBoundingClientRect().right,width:e.getBoundingClientRect().width})).slice(0,8)}))).toMatchObject({width,scroll:width,overflow:[]});}
 const order=await page.evaluate(()=>{const life=document.querySelector('.life-wealth-orbit'),next=document.querySelector('.today-intelligence')??document.querySelector('.today-goals');return !!life&&!!next&&!!(life.compareDocumentPosition(next)&Node.DOCUMENT_POSITION_FOLLOWING);});expect(order).toBe(true);
});
