...
async function showcase(page:Page){
 ...
 await expect(page.locator('.habits-today')).toBeVisible();
}
...
for(const width of [320,360,390,430,768,1440])test(`Habit labels and controls have distinct space at ${width}px`,async({page})=>{
 await page.setViewportSize({width,height:1000});await showcase(page);
 const rows=page.locator('.habit-today-list li');await expect(rows).toHaveCount(3);
 ...
 await page.locator('.habits-today').screenshot({animations:'disabled',path:`/private/tmp/run10-visual-evidence/run10-habits-${width}.png`});
});

test('long build, quit, limit and duration Habit cards reflow with enlarged text and save actions',async({page})=>{
 ...
 await first.screenshot({animations:'disabled',path:'/private/tmp/run10-visual-evidence/run10-long-habit-320.png'});
});

test('category rings, exact history and finite route entrance have visible evidence',async({page})=>{
 ...
 await page.locator('.today-hero').screenshot({animations:'disabled',path:'/private/tmp/run10-visual-evidence/run10-hero-1440.png'});
 ...
 await card.screenshot({animations:'disabled',path:'/private/tmp/run10-visual-evidence/run10-goal-ring-1440.png'});await page.locator('.app-sidebar').screenshot({animations:'disabled',path:'/private/tmp/run10-visual-evidence/run10-sidebar-1440.png'});
 await page.goto('/app/wealth');await expect(page.locator('.wealth-history .evidence-line').first()).toBeVisible();await expect.poll(()=>page.locator('.wealth-history .evidence-line').first().evaluate(el=>getComputedStyle(el).strokeDasharray)).toBe('none');await page.locator('.wealth-history').screenshot({animations:'disabled',path:'/private/tmp/run10-visual-evidence/run10-history-1440.png'});
