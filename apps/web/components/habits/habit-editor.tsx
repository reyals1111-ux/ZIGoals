...
const templates = {
  ...
  buyzig: { title: "Buy ZIG", category: "Finance", type: "build", measurement: "quantity", unit: "USD", target: 500, targetPeriod: "day", schedule: "frequency" },
  ...
  return <section className="panel habit-editor" aria-labelledby={`${formId}-heading`}>
    ...
    <form onSubmit={submit}><fieldset disabled={busy} className="habit-form-fields">
      ...
      <div className="habit-form-grid">
        ...
        <label className="field">Target period<select value={scheduleKind === "frequency" ? "day" : targetPeriod} disabled={scheduleKind === "frequency"} onChange={(event) => setTargetPeriod(event.target.value as HabitRule["targetPeriod"])}><option value="day">Per day</option><option value="week">Per week</option><option value="month">Per month</option><option value="year">Per year</option></select></label>
      </div>
      ...
      {scheduleKind === "frequency" && <div className="habit-form-grid"><label className="field">Times per period<input type="number" min={1} max={366} step={1} value={frequency} onChange={(event) => setFrequency(Number(event.target.value))} /></label><label className="field">Frequency period<select value={frequencyPeriod} onChange={(event) => setFrequencyPeriod(event.target.value as typeof frequencyPeriod)}><option value="week">Week</option><option value="month">Month</option><option value="year">Year</option></select></label></div>}
      ...
      <p className="fine">{habit ? "Type, measurement, recurrence, target, end, pause and archive changes apply today onward. Earlier dates keep their rules." : "Your first scheduled period starts today."} Goal links and stacking are organizational only; no funds move and no automation is implied.</p>
