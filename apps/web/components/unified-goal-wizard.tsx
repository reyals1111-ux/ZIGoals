...
export function UnifiedGoalWizard({positionId}:{positionId?:string}){
 ...
 {isProject?<label className="span-two">Milestones, one per line<textarea value={milestones} onChange={e=>setMilestones(e.target.value)} rows={4}/></label>:<><label>Target amount<input inputMode="decimal" value={target} onChange={e=>setTarget(e.target.value)}/></label>{type==='VALUE'?<label>Goal currency<select aria-label="Goal currency" value={asset} onChange={e=>{setAsset(e.target.value);setAllocation('none');}}><option value="USD">USD · US dollar</option><option value="EUR">EUR · Euro</option></select></label>:<><label>Choose Goal asset<select aria-label="Choose Goal asset" value={goalAssetPosition} onChange={e=>{setGoalAssetPosition(e.target.value);setAsset('');setNetwork('');setAllocation('none');setIncluded({});setSelected('');}}><option value="">Enter an asset symbol</option>{store.data.positions.filter(p=>!p.archivedAt&&(type!=='REWARD'||p.sourceType==='NATIVE_REWARDS')).map(p=><option key={p.id} value={p.id}>{p.asset} · {p.validator?.name??p.providerId} · {p.network}</option>)}</select></label>{!source&&<label>Goal asset<input value={asset} onChange={e=>{setAsset(e.target.value.toUpperCase());setAllocation('none');}} maxLength={30}/><small>Use a symbol such as BTC or ZIG. This creates a target only; add and allocate holdings in Sources.</small></label>}</>}</>}
 <label>Target date (optional)<input type="date" min={localDate()} value={date} onChange={e=>setDate(e.target.value)}/></label><label>Goal notes<textarea value={notes} onChange={e=>setNotes(e.target.value)} maxLength={2000}/></label></div><section className="goal-live-preview" aria-label="Goal preview"><p className="eyebrow">YOUR DESTINATION</p><h3>{name||'Give your next chapter a name'}</h3><strong>{isProject?`${milestones.split('\n').filter(x=>x.trim()).length} milestones`:`${target||'Choose a target'} ${asset}`}</strong><p>{category} · {date||'At your own pace'}</p><small>{isProject?'Only completed milestones count.':type==='REWARD'?'Only currently unclaimed rewards count.':type==='VALUE'?'Only allocated wealth in this currency counts.':'Only allocated units of the chosen asset count.'}</small></section></>}
