...
export function UnifiedGoalWizard({positionId}:{positionId?:string}){
 ...
 const [pendingType,setPendingType]=useState<PrivateGoal['type']|null>(null);
 ...
 function draft(id=previewId):PrivateGoal{
  if(goalAssetPosition&&(!source||source.archivedAt))throw Error('The chosen asset is no longer available. Return to Purpose and choose another asset.');
 ...
 function validatePurpose(){
  ...
  if(isProject){if(!milestones.trim())throw Error('Add at least one milestone.');}else if(BigInt(parseUnits(target,precision))<=0n)throw Error('Enter a positive target.');
 }
 function next(){try{
 ...
 ] as const).map(([value,label,description])=><label key={value}><input type="radio" name="goal-type" aria-label={label} checked={(pendingType??type)===value} onChange={()=>requestType(value)}/><span><strong>{label}</strong><small>{description}</small></span></label>)}</fieldset>
 {pendingType&&<section className="goal-type-confirm span-two" aria-label="Confirm Goal type change"><h3>Change how this Goal is measured?</h3><p>Your name, category, notes and date stay. The target, milestones, selected allocations and contribution plan will be cleared so they cannot be reinterpreted as a different kind of progress.</p><div className="goal-choice-actions"><button type="button" className="secondary" onClick={()=>setPendingType(null)}>Keep current type</button><button type="button" className="primary" onClick={()=>changeType(pendingType)}>Change type</button></div></section>}
 {isProject?<label className="span-two">Milestones, one per line<textarea value={milestones} onChange={e=>setMilestones(e.target.value)} rows={4}/></label>:<><label>Target amount<input inputMode="decimal" value={target} onChange={e=>setTarget(e.target.value)}/></label>{type==='VALUE'?<fieldset className="goal-choice-tiles"><legend>Goal currency</legend>{(['USD','EUR'] as const).map(currency=><label key={currency}><input type="radio" name="goal-currency" value={currency} checked={asset===currency} onChange={()=>{setAsset(currency);setAllocation('none');}}/><span>{currency} · {currency==='USD'?'US dollar':'Euro'}</span></label>)}</fieldset>:<fieldset className="goal-choice-tiles span-two"><legend>Choose Goal asset</legend><label><input type="radio" name="goal-source" value="manual" checked={!goalAssetPosition} onChange={()=>{setGoalAssetPosition('');setAsset('');setNetwork('');setAllocation('none');setIncluded({});setSelected('');}}/><span>Enter an asset symbol</span></label>{store.data.positions.filter(p=>!p.archivedAt&&(type!=='REWARD'||p.sourceType==='NATIVE_REWARDS')).map(p=><label key={p.id}><input type="radio" name="goal-source" value={p.id} checked={goalAssetPosition===p.id} onChange={()=>{setGoalAssetPosition(p.id);setAsset('');setNetwork('');setAllocation('none');setIncluded({});setSelected('');}}/><span>{p.asset} · {p.validator?.name??p.providerId} · {p.network}</span></label>)}</fieldset>}{type!=='VALUE'&&!source&&<label>Goal asset<input aria-label="Goal asset" value={assetInput} onChange={e=>{setAsset(e.target.value.toUpperCase());setAllocation('none');}} maxLength={30} placeholder="ZIG"/><small>Use a symbol such as BTC or ZIG. This creates a target only; add and allocate holdings in Sources.</small></label>}</>}
 <label>Target date (optional)<input type="date" min={localDate()} value={date} onChange={e=>setDate(e.target.value)}/></label><details className="goal-optional span-two"><summary>Personalize artwork and notes (optional)</summary><fieldset className="goal-choice-tiles"><legend>Category / artwork</legend>{CATEGORIES.map(c=><label key={c}><input type="radio" name="goal-category" checked={category===c} onChange={()=>setCategory(c)}/><span>{c}</span></label>)}</fieldset><label>Goal notes<textarea value={notes} onChange={e=>setNotes(e.target.value)} maxLength={2000}/></label></details></div><section className="goal-live-preview" aria-label="Goal preview"><p className="eyebrow">YOUR DESTINATION</p><h3>{name||'Give your next chapter a name'}</h3><strong>{isProject?milestones.split('\n').filter(x=>x.trim()).length?`${milestones.split('\n').filter(x=>x.trim()).length} milestones`:'Add a milestone':target.trim()?`${target} ${asset}`:'Choose a target'}</strong><p>{type[0]+type.slice(1).toLowerCase()}{isProject?'':` · ${asset}`} · {category} · {date||'At your own pace'}</p><small>{isProject?'Only completed milestones count.':type==='REWARD'?'Only currently unclaimed rewards count.':type==='VALUE'?'Only allocated wealth in this currency counts.':'Only allocated units of the chosen asset count.'}</small></section></>}
 ...
 <div className="wizard-actions"><button className="secondary" disabled={step===0||busy||!!savedId} onClick={()=>{setError('');setStep(step-1);}}>Back</button>{step===0?<><button className="secondary" disabled={!store.loaded||busy||!!pendingType} onClick={next}>Continue →</button><button className="primary" disabled={!store.loaded||busy||!!pendingType||!!store.error} onClick={()=>void save()}>{busy?'Saving…':'Create goal'}</button></>:step<3?<button className="primary" disabled={!store.loaded||busy||!!pendingType} onClick={next}>Continue →</button>:<button className="primary" disabled={busy||!!reviewError||(isLocal?legacy.busy||!legacy.canTransact:!!store.error||!store.loaded||(withHabit&&wantsPlan&&(!habits.loaded||!!habits.error)))} onClick={()=>void save()}>{busy?'Saving…':'Create goal'}</button>}</div>
 </div></section>;
}
