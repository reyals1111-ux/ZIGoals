'use client';
import {ManualSourceCards} from './platform/manual-source-cards';
...
export function UnifiedGoalWizard({positionId}:{positionId?:string}){
 ...
 const market=useMarketQuotes(type==='VALUE'&&asset==='USD'&&network==='zigchain-1'&&funding==='wealth');
 ...
 {funding==='wealth'&&<ManualSourceCards update={store.update} onSaved={p=>{setSelected(p.id);setQuantity(amount(p.quantity,p.decimals));setAllocation(type==='VALUE'||assetMatches(identity,p)?'position':'none');}}/>}
 ...
 {type==='VALUE'&&<p className="fine">Value Goals count allocated wealth in the Goal currency. Native mainnet ZIG is valued automatically in USD using verified market quotes. Other assets need an explicit valuation; plans never count as current wealth.</p>}{type==='REWARD'&&<p className="fine">Reward Goals count currently unclaimed rewards, not cumulative claimed income.</p>}</>}</>}
