...
export function UnifiedGoalWizard({positionId}:{positionId?:string}){
 ...
 {funding==='wealth'&&<ManualSourceCards update={store.update} onSaved={p=>{if(type==='VALUE')setIncluded(old=>({...old,[p.id]:amount(p.quantity,p.decimals)}));setSelected(p.id);setQuantity(amount(p.quantity,p.decimals));setAllocation(type==='VALUE'||assetMatches(identity,p)?'position':'none');}}/>}
