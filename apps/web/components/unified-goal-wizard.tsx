...
export function UnifiedGoalWizard({positionId}:{positionId?:string}){
 ...
 const [withPlan,setWithPlan]=useState(false),[planAmount,setPlanAmount]=useState(''),[planAsset,setPlanAsset]=useState('ZIG'),[cadence,setCadence]=useState<'weekly'|'monthly'|'yearly'|'irregular'>('monthly'),[nextDate,setNextDate]=useState(localDate),[price,setPrice]=useState(''),[withHabit,setWithHabit]=useState(false);
 ...
 function draft(id=previewId):PrivateGoal{
  ...
  const plan=wantsPlan?contributionSchema.parse({amount:parseUnits(planAmount,pDecimals).toString(),asset:planAsset,decimals:pDecimals,cadence,nextDate,active:true,price:price?{value:parseUnits(price,18).toString(),decimals:18,currency:planAsset}:undefined}):undefined;
 ...
 const eligible=store.data.positions.filter(p=>(p.network==='manual'||p.network===network)&&(p.providerId!=='native-zig'||(scope?p.account===scope.account:nativeAccounts.length<=1))&&(type==='VALUE'||assetMatches(identity,p)&&(type!=='REWARD'||p.sourceType==='NATIVE_REWARDS')));
 ...
 if(step===3&&!isLocal)try{reviewGoal=savedId?store.data.goals.find(g=>g.id===savedId):draft();if(reviewGoal)review=goalProgress(savedId?store.data:preparePrivate(store.data,reviewGoal),reviewGoal.id);}catch(e){reviewError=e instanceof Error?e.message:'Review allocations.';}
 ...
 {funding==='wealth'&&<div className="setup-next"><h3>Existing wealth</h3>{source&&<p>From {source.validator?.name??source.providerId} · {source.asset}. Choose an allocation below; nothing is reserved yet.</p>}{scope&&<p className="fine">Snapshot account: {scope.account}</p>}<p>Available stake: <strong className="nebula-number">{amount(availableStake)} ZIG</strong></p>{stake.length>0&&<label className="checkbox"><input type="checkbox" checked={allocation==='stake'} onChange={e=>setAllocation(e.target.checked?'stake':'none')}/>Allocate available stake up to Goal target</label>}
 <label className="checkbox"><input type="checkbox" checked={allocation==='position'} onChange={e=>setAllocation(e.target.checked?'position':'none')}/>Allocate a specific Position</label>
 <label className="field">Source Position<select value={selected} onChange={e=>setSelected(e.target.value)}><option value="">Choose a Position</option>{eligible.map(p=><option key={p.id} value={p.id}>{p.validator?.name??p.providerId} · {p.sourceType.replaceAll('_',' ')} · {amount(allocationBalance(store.data,p.id).unallocated,p.decimals)} {p.asset} available</option>)}</select></label>
 {allocation==='position'&&<label className="field">Units to allocate<input inputMode="decimal" value={quantity} onChange={e=>setQuantity(e.target.value)}/></label>}
