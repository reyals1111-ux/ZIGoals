...
export function ActivityFeed({ limit = 6, category = "ALL", includeGoals = true }: { limit?: number; category?: string; includeGoals?: boolean }) {
  const goals = useGoals(), habits = useHabits(), health = useHealth();
  const entries = [
    ...(includeGoals && goals.mode === "local" ? goals.activity.map((event, index) => ({ id: `goal-${event.timestamp}-${index}`, category: "GOAL", title: `Goal ${event.action}`, detail: goals.metadata?.goals[event.goalId]?.name ?? `Goal #${event.goalId}`, at: event.timestamp, href: `/app/goals/${event.goalId}` })) : []),
    ...getHabitActivities(habits.data), ...getHealthActivities(health.data),
  ].filter(event => category === "ALL" || event.category === category).sort((a, b) => b.at.localeCompare(a.at)).slice(0, limit);
