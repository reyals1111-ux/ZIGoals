...
export function VaultSyncProvider({children}:{children:ReactNode}){
 ...
 function forget(){setAttachPreview(null);setPendingRecovery(null);session.current=null;auto.current=false;setOpened(false);setGenerated(null);setManifest(undefined);setAccount(null);setHealthState(false);setMessage('Account sync is locked.');setLast('');setError('');}
 ...
 useEffect(()=>{
  ...
  const change=()=>{if(isAccountLocked()||(session.current&&getAccountScope()!==session.current.account)){session.current=null;auto.current=false;setAttachPreview(null);setPendingRecovery(null);setOpened(false);setGenerated(null);setManifest(undefined);setAccount(null);setHealthState(false);setError('');setLast('');setMessage('Account sync is locked.');}};
