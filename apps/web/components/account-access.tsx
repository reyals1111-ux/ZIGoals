...
export function AccountAccess({onAuthenticated,onSignout}:Props){
 ...
 async function submit(action:'send'|'verify'){
  ...
  try{
   ...
   const generation=getAccountGeneration();const res=await fetch('/api/private-account',{method:'POST',headers:{'content-type':'application/json'},cache:'no-store',signal:controller.signal,body:JSON.stringify(action==='send'?{action,email:address}:{action,email:address,code})});
