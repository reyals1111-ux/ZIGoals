...
import {ringSegments,type AssetMix} from '../lib/wealth';
...
export function GoalProgressRing({name,progressPct,goalId,assetMix=[]}:{assetMix?:AssetMix[];name:string;progressPct:string;goalId?:string}){
 const segments=ringSegments(assetMix,progressPct);
 const palette=goalPalette(goalId??name);
 const ringId=useId();const pct=Math.max(0,Math.min(100,Number(progressPct)));
 return <div className="goal-progress-ring" role="progressbar" aria-label={`${name} progress`} aria-valuemin={0} aria-valuemax={100} aria-valuenow={pct}><svg viewBox="0 0 88 88" aria-hidden="true" focusable="false"><defs><linearGradient id={ringId} x1="0" y1="0" x2="1" y2="1"><stop stopColor={palette[0]}/><stop offset="1" stopColor={palette[1]}/></linearGradient></defs><circle className="ring-track" cx="44" cy="44" r="38"/>{segments.length>1?segments.map(segment=><circle key={segment.assetClass} className="ring-value" style={{stroke:segment.color,strokeLinecap:"butt"}} cx="44" cy="44" r="38" pathLength="100" strokeDasharray={`${segment.size} ${100-segment.size}`} strokeDashoffset={-segment.offset} transform="rotate(-90 44 44)"/>):<circle style={{stroke:`url(#${ringId})`}} className="ring-value" cx="44" cy="44" r="38" pathLength="100" strokeDasharray={`${pct} 100`} transform="rotate(-90 44 44)"/>}</svg><strong aria-hidden="true">{new Decimal(progressPct).toFixed(2,Decimal.ROUND_DOWN)}%</strong></div>;
}
