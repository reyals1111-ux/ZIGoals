...
export function PriceChart({marketRef,currency,localPoints=EMPTY_POINTS,title='Price history'}:PriceChartProps){
 ...
 const groups:number[][]=[];
 points.forEach((point,index)=>{const group=groups.at(-1),last=group?.at(-1);if(last===undefined||Date.parse(point.at)-Date.parse(points[last]!.at)>7*86400000)groups.push([index]);else group!.push(index);});
 ...
 return <figure ref={entrance} className="price-chart" aria-label={title}>
  ...
  {points.length?<>
   {points.length>1&&<svg className="price-chart-plot" viewBox="0 0 760 256" role="img" tabIndex={points.length>1?0:undefined} aria-labelledby={`${id}-title`} onPointerMove={event=>{const rect=event.currentTarget.getBoundingClientRect();inspect(event.clientX,rect.left,rect.width);}} onPointerLeave={()=>setInspectedAt(null)} onKeyDown={event=>{if(!['ArrowLeft','ArrowRight','Home','End'].includes(event.key))return;event.preventDefault();const at=Math.max(0,points.findIndex(p=>p.at===current?.at)),next=event.key==='Home'?0:event.key==='End'?points.length-1:Math.max(0,Math.min(points.length-1,at+(event.key==='ArrowLeft'?-1:1)));setInspectedAt(points[next]!.at);}}>
