...
export function AssetPicker({positions=[],onExisting,onDraft,onFavourite,initialCategory='Crypto',submitLabel='Use this asset'}:{positions?:Position[];onExisting?:(p:Position)=>void;onDraft?:(p:Position)=>void;onFavourite?:(a:MarketCatalogAsset)=>void;initialCategory?:string;submitLabel?:string}){
 ...
 const insights=useMarketInsights([...positions.flatMap(p=>p.marketRef?[p.marketRef]:[]),...(selected?[selected.ref]:[])]);
 ...
 {category==='Existing assets'?<div className="picker-existing">{positions.filter(p=>!p.archivedAt).map(p=><button key={p.id} type="button" onClick={()=>onExisting?.(p)}><AssetIcon symbol={p.asset} kind={p.assetClass} logoUrl={p.marketRef?insights.results[marketRequestKey({marketRef:p.marketRef,currency:'USD'})]?.insight?.logoUrl:null}/><span><strong>{p.providerId}</strong><small>{amount(p.quantity,p.decimals)} {p.asset} · {p.valuation?.source==='MANUAL'||p.valuationMode==='manual'?'Manual value':p.marketRef&&p.valuationMode==='automatic'?'Automatic price':p.valuation?'Recorded value':'Needs valuation'}</small></span></button>)}{!positions.filter(p=>!p.archivedAt).length&&<p>No assets yet. Choose a category to add your first.</p>}</div>:<>
 ...
 {selected&&automatic&&<div className="picker-selected wide"><AssetIcon symbol={selected.symbol} kind={category} logoUrl={insights.results[marketRequestKey({marketRef:selected.ref,currency:'USD'})]?.insight?.logoUrl}/><div><h3>{selected.name}</h3><p>{selected.symbol.toUpperCase()} · {category} · {selected.ref.kind==='rwa'?'CoinGecko tokenized RWA reference':'CoinGecko'}</p></div></div>}
