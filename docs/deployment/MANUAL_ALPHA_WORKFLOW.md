...
## Historical baseline and deployment authority
...
The checked routes are `/app`, `/app/habits`, `/app/health`, `/app/goals`, `/app/goals/new`, `/app/wealth`, `/app/markets`, `/app/activity`, `/app/ecosystem` and `/app/settings`, plus a repeated `/app` request for nonce freshness. Smoke GETs target only `https://alpha.zigoals.app`; no wallet, personal browser profile, test data, chain request or apex request is involved. Identical duplicate HSTS/X-Robots-Tag values remain accepted; the existing CSP/nonce architecture is unchanged.
...
## Run #9.2 release evidence

[PR #17](https://github.com/reyals1111-ux/ZIGoals/pull/17) delivered Run #9/#9.1/#9.2; [PR #18](https://github.com/reyals1111-ux/ZIGoals/pull/18) hardened protected CoinGecko runtime-secret publication. Manual run `35655131172` uploaded source `702762d079be1f74536219c027c4d267bfd33fc0` as live Worker `cca2b972-3b40-47a6-affe-1249fe260465`, retaining rollback `30468b51-fb8d-4f9e-bd6c-b36d4a9f89e5`.

That run ended `NEEDS_OWNER_REVIEW` because smoke still expected the obsolete V2.1 hero “Turn today”. The Run #9.2 hero and exact Settings SHA were subsequently verified live; that historical false negative is not a rollback instruction. Current smoke expects “YOUR FINANCIAL ORBIT” and includes Wealth and Markets, retaining exact-source, safety-mode, redirect, CSP and fresh-nonce checks.

The release-closure patch adds CoinGecko Ethereum token-address pricing only when native ZIG (`zignaly`) ID-based quotes fail. Other assets keep ID-based pricing. This does not add a history/insights fallback or another provider. Final closure deployment subsequently completed as run `35702856008` (SUCCESS / VERIFIED). Any future deployment still requires its own exact-main CI pass and protected owner approval. See [release evidence](../verification/run9-2-release/README.md).

## Run #10 market acceptance is a separate evidence track

Existing deployment/security/source smoke remains mandatory. A successful exact-source deployment and security smoke do not establish market API readiness. A future approved market runner must separately validate Bitcoin quote, native ZIG quote, both catalog partitions and supported Bitcoin history, without exact price assertions or private data. The pure classifier is `apps/web/lib/market-acceptance.ts`; this foundation does not connect it to the deployment workflow.

Feed the classifier only canonical, validated evidence: VERIFIED means correct identity, fresh timestamps, valid exact decimals, and complete catalog partitions where relevant; stale/partial results remain degraded. HTTP status alone is insufficient. Record reachability before attributing failures: an explicit edge access denial (including HTTP 403 / Cloudflare Error 1010), unreachable client or unconfirmed application response means BLOCKED. Do not reinterpret BLOCKED as provider unavailable/throttled or application market failure. A generic 403 alone does not establish authentication or entitlement failure. Preserve the blocked runner evidence and obtain an approved runner context; do not automatically retry public endpoints or roll back.

Run #9.2 final deployment `35702856008` was SUCCESS / VERIFIED (11/11 security/source smoke). Its separate market incident remains UNCONFIRMED; see [final evidence](../verification/run9-2-release/README.md). Documentation changes do not authorize or require deployment.
