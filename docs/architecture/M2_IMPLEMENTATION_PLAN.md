...
- [ ] Attempt connected GitHub write once; publish/prepare PR and observe CI if enabled; otherwise record exact blocker.
- [ ] Fresh live testnet evidence; check Docker and clean isolated Wasm artifact/toolchain equality.
...
- [ ] Write targeted failing persistence/restart/account/corruption/uncertainty tests before implementing.
- [ ] Implement validated version1 IndexedDB records, atomic transitions and pre-broadcast durable barrier.
- [ ] Compute/retain signed transaction hash when possible and verify known receipts against operation identity.
- [ ] Integrate provider/outcome/activity UI, preserving M1 behavior and explicit history completeness.
- [ ] Cover browser restart and damaged/scoped history; typecheck/test/lint, self-review and commit only owned files.
- [ ] Independent task review; repair Important findings and re-review.
...
- [ ] Protocol leg: Valdora, OroSwap, PermaPod, Nawa, ZIG Markets, Zignaly, WME; execution schemas, security/eligibility and explicit gates.
- [ ] Infrastructure leg: Zamanat, DeFa/InvoiceMate, Beehive, Ondo, Taurus, Apex, Noble, Axelar; actual ZIGChain relationship versus announcement, roles and funding/strategy distinction.
- [ ] Controller: Range, ZIGScan and Hub, observed route evidence and network scope.
- [ ] Consolidate each participant's role/network/API/risk/eligibility/status/date/use/phase with primary links; never infer integrations from logos.
...
- [ ] Write failing unsafe URL/identifier/unknown-capability/trust-promotion tests.
- [ ] Implement provider schema/data, per-capability verified route builders and safe Hub link catalogue.
- [ ] Expand StrategyDescriptor compatibly, FundingRoute separately and evidence-only provenance display.
- [ ] Integrate verified read-only links after Task2-owned UI files are stable; responsive/browser checks.
- [ ] Independent review of package/interface boundaries and actionable links.

## Task 5 — Readiness/security/handoff (controller)

- [ ] Add precise human Keplr and testnet deployment checklist with immutable admin/checksum/receipt/event/abort gates.
- [ ] Update threat model/checklist and draft-only Build Log; prepare local PR description if access blocked.
