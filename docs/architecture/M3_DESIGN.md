...
2. Run full PR quality checks and main pushes. Feature pushes use a PR-presence gate to avoid duplicate full suites where practical while still testing a branch before PR creation. Preserve every existing quality gate. Publish a pinned-toolchain Wasm plus machine-readable size/hash/environment report as a CI artifact; hash differences are observations pending investigation, not automatic release verdicts.
