...
- [x] **2 Shell:** `components/shell.tsx` plus shell CSS; seven real routes, desktop sidebar/mobile core navigation, existing exact wallet handlers/dialogs and safety text. Existing wallet-reload/route tests; checkpoint.
- [x] **3 Today:** `app/app/page.tsx`, Today components; reference composition, truthful Goal summaries, daily Habit actions, Health summary and recent local activity. Implement connected cards after module data is available; new persistence is an explicit dependency adjustment ahead of final Today wiring. Empty/populated layout tests; checkpoint.
- [x] **4 Goals:** dedicated `app/app/goals/page.tsx`; update `goal-card.tsx`, wizard/detail presentation and scoped supporting Habit links. Preserve old URLs and exact financial math/actions. Run existing Goal lifecycle and damaged-storage tests; checkpoint.
- [x] **5 Habits:** `lib/habits.ts`, `lib/habits.test.ts`, `components/habits/`, `app/app/habits/page.tsx`; strict schema, daily/weekdays, CRUD/pause/archive, counted completions, local-date streaks and heatmap, scoped Goal links. Test daily/weekdays, today grace, missed/not scheduled, pauses, DST/date boundaries, reload/corruption/imports; checkpoint.
