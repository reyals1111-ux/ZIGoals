# Run #9.1 remaining Beta backlog

Delivered behavior and verification are in [Run #9.1 report](RUN_9_1_REPORT.md). These items are deliberately distinct from working features.
...
| Next | Distributed CoinGecko quota admission | Existing process-local 12/minute and 2 concurrent limits are shared by catalog, quotes and history; multiple instances still multiply provider usage. |
| Next | Provider-backed 24h change and watchlist sparklines | Show only after actual provider evidence is added to the strict quote contract/cache. Do not infer change from missing days or fetch history for every card eagerly. Detail-chart change already compares its actual shown observations. |
...
| Next | Portfolio capacity and richer asset navigation | Eight favourites are supported. Add lower-frequency category summaries, search/sort of owned assets and further mobile density options for large portfolios. |
...
| Separate approval | Hosted rollout | Requires an explicit release task, clean reviewed commit and environment-bound smoke checks. No deployment, merge or push is part of Run #9.1. |
