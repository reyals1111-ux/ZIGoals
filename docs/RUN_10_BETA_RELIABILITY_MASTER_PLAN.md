# ZIGoals — PRE-RUN #10: Reliability Hardening / P2 Closure

## 1. Objective
Reliable Market Evidence for Beta: preserve independently verified public evidence, truthfully expose degradation, and prepare account-wide capacity control without changing financial semantics. This draft PR is PRE-RUN #10 foundation work only. Actual Run #10 has NOT started: it remains a future major product/platform milestone driven by one large owner-approved master prompt, with substantial implementation, UX/UI refinement, reliability/security/performance work, testing and release preparation. No deployment or production coordinator activation is included here.
...
## 3. Current evidence
This plan consolidates the owner-supplied completed preflight Beta Hardening Audit and preflight Market Reliability Investigation findings, checked against source at `95ff4d3ea3e0d8c2c33b497bdcefaac0cc539a90`. The audits are supplied in the original foundation brief; no nonexistent repository audit artifact is implied.
...
Immediate market acceptance: native ZIG 503; Bitcoin quote 200 / VERIFIED; catalog 503; Bitcoin history 503. The preflight investigation’s cloud client was subsequently blocked with HTTP 403 / Cloudflare Error 1010 / access_denied before usable market evidence reached it.
...
Pure `market-budget-policy.ts` is disconnected from active admission. Inject provider minute request limit and monthly credit entitlement separately from internal operating ceilings, concurrency, monitoring reserve/maximum, optional/background cutoff, request credit cost, retry/fallback kind, priority, time and calendar-month identity/boundaries. No daily provider allowance or counter is required or invented. Missing/invalid mandatory configuration denies reservations. Test numbers are synthetic, never production defaults. Operation IDs are server-created opaque tokens, never wallet/user identifiers.

## 11. Future distributed coordinator architecture
One serialized durable authority per credential/account budget spanning all Worker instances and endpoints. Account selection is server configuration, never caller credentials. Operations: enqueue, reserve one multi-key provider attempt, ownDispatch, markDispatched, settle, cancelUndispatched, acquire/release work lease, fenced publishResult, scoped breakerState and recordOutcome. One batch of 250 IDs is one attempt/minute request with configurable monthly credit cost, not 250 provider requests. No production persistence, network or binding now. Future implementation must prove transactional idempotency, restart recovery, fencing and safe tombstone retention. A Durable Object alone is not a shared-cache strategy.

## 12. Cache-before-admission
Future sequence: validate → shared last-good cache → fresh hits → coalesce misses → bounded queue → budget reservation → dispatch ownership → durable DISPATCHED mark → provider I/O → settlement → per-work fenced publication. Fresh hits cost zero provider work. Stale evidence may return with degradation. Hydration must never advance evidence timestamps. Existing cache gates remain; no distributed activation here.
...
## 14. Cost reservation
Reserve before I/O, durably markDispatched before network send. QUEUED → RESERVED → OWNED → DISPATCHED → SETTLED; cancellation is allowed only before DISPATCHED. Queued work costs no capacity; reservations hold minute/month budget without concurrency; OWNED and DISPATCHED hold concurrency. Dispatched failures remain charged. Expired undispatched owners require explicit cancellation; uncertain dispatched I/O requires settlement, never automatic slot reuse. Expired or previous-period holds require cancellation and a new reservation before dispatch. IDs cannot be reused after cancellation. Production pruning waits for an explicit idempotency-retention policy.
...
## 16. Priorities
Interactive, refresh, optional and monitoring classes. Future scheduler makes optional/prefetch yield under pressure and bounds fairness/starvation. Non-monitoring usage has its own ceiling excluding the monitoring reserve. Monitoring has its own cap strictly below each operating ceiling; total usage also obeys that ceiling and provider entitlement. Accepted holds are not re-admitted at dispatch, so later monitoring cannot revoke them. Optional/refresh work hits a lower configurable cutoff first. No final operating percentages are selected; numerical test policies are synthetic. Scheduler ordering/starvation guarantees still require implementation and workload tests.

## 17. Circuit breakers
Pure CLOSED/OPEN/HALF_OPEN transitions scope account authentication, independently proven account throttle, endpoint availability, endpoint integrity and individual pairs separately. LOCAL_BUDGET, LOCAL_QUEUE and BLOCKED never increment provider counters, even for admitted probes. Unknown/internal failures also do not blame the provider. Inject threshold, observation window, cooldown, maximum cooldown and half-open probe count; fence late settlement and require all admitted recovery probes to succeed. Future adapter must budget probes and settle canceled/abandoned probes explicitly. No breaker activation, unlimited retries or provider substitution now.
...
Inability to reach the application or explicit edge denial (including Error 1010) means BLOCKED. It is not application market failure or provider unavailability/throttling. Arbitrary 403 alone does not prove who denied access. Preserve APPLICATION_REACHED / EDGE_BLOCKED / UNREACHABLE / UNKNOWN, security PASS/FAIL/NOT_CHECKED, deployment identity PASS/FAIL/NOT_CHECKED, and all four per-probe VERIFIED/STALE/PARTIAL/UNAVAILABLE/THROTTLED/MALFORMED/NOT_RUN values in the acceptance report alongside its summary. BLOCKED leaves market acceptance outstanding and calls for runner review, never automatic rollback.
...
2. Owner confirms internal operating policy, exact reset timezone, cache and runner decisions against the recorded account facts.
...
Owner-confirmed CoinGecko entitlements and exclusive credential usage (recorded below); exact reset boundary timezone still needs verification before runtime accounting activation; approved runner reaching Alpha; shared-cache choice; Cloudflare persistence/lease semantics; owner release/security review. No second provider approved.

## 29. Owner decisions still required
Internal operating ceilings, monitoring reserve and maximum, optional cutoff, endpoint credit weights, exact monthly reset timezone, activation policy, cache backend and approved market-smoke runner. Minute/month entitlement and exclusive key usage are now owner-confirmed; no daily provider limit exists in the verified evidence. Also measured workload/SLOs, operational retention/access and idempotency retention. Never substitute illustrative planning numbers.
...
## Preflight review dispositions and activation gates
...
| --- | --- |
| P2-01 — lease expiry / accumulated results | PURE CONTRACT CLOSED; exact-boundary/fence/follower/partial-evidence tests, plus truthful late-completion degradation in existing caches. Production atomic persistence remains an activation gate. |
| P2-02 — monitoring reserve/fairness | PURE SEMANTICS CLOSED; independent reserve/cap/total ceilings, stable accepted reservations, optional cutoff and queue/reservation/dispatch ownership tests. No production scheduler/global admission. |
| P2-03 — body/parser misclassification | CLOSED; explicit expected validation errors and narrow body-transport boundaries. Internal exceptions are UNKNOWN; public failure messages remain generic. |
| P2-04 — response-body cancellation | CLOSED; rejected responses are canceled without parsing; bounded best-effort cleanup precedes permit release and cannot replace the original failure. |
| P2-05 — coordinator interface | PURE CONTRACT CLOSED; one multi-key provider attempt/reservation, per-work fences/publication, one charged dispatch and settlement. No durable adapter. |
| P2-06 — acceptance model | PURE CONTRACT CLOSED; raw multidimensional evidence is copied into a report with a derived summary. No deployment-gate wiring. |
| P2-07 — privacy disclosure | ADDRESSED IN THE PRIOR P1 CORRECTION: PRIVACY describes public market requests and selection/timing linkability without claiming anonymity. |
...
## Owner-confirmed CoinGecko account and preflight milestone

The owner inspected the Developer Dashboard on 2026-09-22: Demo plan, **100 calls/minute**,
**10,000 call credits/month**, **171 used / 9,829 remaining (1.7%)** at that snapshot.
The dashboard shows calendar-month replenishment and next reset **October 1, 2026**.
The key is used **only by ZIGoals**. These are observed account facts, not a live usage feed.
The Demo `/api/v3/key` request returned HTTP 401 / error 10005, PRO-only; Demo accounting
must not depend on it. The provider dashboard is the entitlement authority; runtime internal
accounting must be conservative, seeded/reconciled against the dashboard before activation.
No verified provider daily limit exists and none is invented. Endpoint/retry/fallback credit
costs remain configurable; no promise is made that all future endpoints have equal weights.
The pure algorithms contain no invisible production entitlement defaults. Runtime policy can
explicitly supply providerMinuteLimit=100 and providerMonthlyLimit=10000; operating ceilings,
monitoring reserve/maximum and optional cutoff remain owner decisions.

Owner-Mac Alpha retest: native ZIG 503, Bitcoin quote 200/VERIFIED, catalog/history/insights 503.
Controlled direct requests with the same credential returned 200 for Bitcoin and ZIG simple
price, ZIG token-address fallback, coins/list, rwas/list, Bitcoin market_chart and coins/markets.
The provider, credential, identities and tested entitlements worked during that comparison.
The precise internal deployed Alpha/runtime root cause is still UNCONFIRMED. No claim that
this hardening fixes it, no rollback and no speculative provider substitution.

All work here is PRE-RUN #10. The historical PR title/branch and verification directory names
are retained as identifiers, not new numbered product runs. Run #9.2 remains the approved
current visual/product baseline; future owner-approved major work may improve the UI.
See [checkpoint and verification record](PRE_RUN_10_HARDENING.md).
