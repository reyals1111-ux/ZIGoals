...
| P04 | Visual widget library and source-card pinning | Partial: visual group/tile/source/metric selection and eligible Goal/Habit/asset/Position/Health/Wealth/ecosystem source menus implemented. No-source staking hidden; bound Habit updates; distinct bindings/reload/duplicate refusal pass desktop/mobile. Missing-source edit/recovery, wider source-module inventory and final integration remain. |
| P05 | Illustrated cards and quiet accessible settings menus | Partial: widget and source cards now use compact focusable option menus; 320px opening/Escape focus return and source integrity tested. Purpose-specific artwork, all menu action/focus paths, zoom and final visual acceptance remain. |
...
P04 source-module inventory at C: registered aggregates are Goals, Habits, Health meals/macros/water/weight/steps/activity/30-day nutrition, Wealth per-currency current/history, ecosystem directory; registered bound records are individual Goal, Habit, asset, eligible staking/reward Position and Position allocation, plus official ecosystem project. Menus exist on Goal/Habit, Health daily/water/weight/activity/nutrition history, Wealth asset/subtotal/history, Position and ecosystem project cards. Food-entry or per-meal-group pinning, any remaining visible trackable source modules, missing-source recovery UI, and full no-data/locked testing stay **incomplete**; the safe registry does not imply arbitrary cards are supported. `dashboard-widget-registry.ts` controls eligibility and `dashboard-metrics.ts` computes from canonical selectors; no external embed or arbitrary code is accepted.
