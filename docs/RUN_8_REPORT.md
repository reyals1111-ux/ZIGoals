...
- `/app/goals` is one V2.1 card collection with Active / Completed / Closed / All. `/app/goals/new` is the shared four-step creator; Position shortcuts carry source context without allocating automatically. Old detail and tracked collection URLs still work.
- Quantity / Value / Reward / Project, explicit existing-wealth allocation, future contribution plans and optional supporting Habits use private accounting. Local simulation retains its existing confirmation, ledger and recovery path. The two models/stores intentionally remain separate; no migration or contract change. Optional category metadata preserves chosen artwork.
- False Completed root cause: an unrestricted **Mark completed** toggle wrote the status flag independently of counted progress, and filters trusted it. Removed that action. Schema reads/imports now derive non-closed status with integer comparison against counted progress (all milestones for Projects); next explicit private edit persists reconciliation. Partial/reduced balances return Active, closed stays Closed, and allocations/snapshots/history are preserved. Original stored bytes are untouched on read. Milestone edits capture checkbox state before the asynchronous save, so Project completion persists.
