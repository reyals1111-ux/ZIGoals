...
For a hosted preview, use only the verified URL listed in [current status](../STATUS.md). The official marketing domain is **zigoals.app**. Do not assume a planned Alpha hostname is live or trust a similar-looking domain. Read the [Alpha disclaimer](ALPHA_DISCLAIMER.md) and [privacy notes](../PRIVACY.md) before entering information; use fictional plans for testing.
...
Plans and history belong to the browser origin. Switching from a preview URL to a custom domain starts a separate storage scope. Export/import a private planning backup yourself if needed; neither the host nor maintainers need the file. The web host sees request paths, including goal IDs, while plan text remains local in the supported flows. No frontend rollback recovers deleted browser data.
