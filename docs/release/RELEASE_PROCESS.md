...
3. Wait for every gate: existing lint/typecheck/unit/production-build/browser/network-doctor/secret checks; Rust format/Clippy/tests/schema drift; production npm audit; current RustSec audit; two fresh canonical builds; strict byte/source/shared-environment comparison with both runner image versions retained; downloaded-candidate validation; official attestation. The audit compiler is Rust 1.88.0 for cargo-audit 0.22.2 only; contract builds still use 1.85.1. Known unmaintained-crate notices remain visible; vulnerability findings block the audit job without an ignore list.
...
## Manifest v2 transition (Run #10)

New builds use schema 2 / `canonical-linux-v2`. Only the moving GitHub image version may differ, and each builder must retain it in `builds[].runner.imageVersion`. All other environment fields, source/tree/lock/epoch and actual artifact bytes/hash/size still match exactly; the trusted validator is rerun before output publication. See [ADR-005](../architecture/ADR-005-canonical-release-build.md) for the bounded policy and rationale.

Current comparison, downloaded-candidate verification and deployment preparation reject old/mixed/future versions. To verify the historical v1 candidate above, use the separately reviewed v1 verifier/schema from `709816a7e6d0779b45c48d729c69e0eabd671f0e`, preserving the original trusted candidate source and attestation constraints. Never edit an old manifest, copy a host version between jobs or convert a failed old run into a v2 success. Obtain new v2 evidence through a new exact-source run; manual issuance and upload remain owner-controlled.
