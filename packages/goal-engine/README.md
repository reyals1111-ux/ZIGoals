...
Monthly growth factor `q = (1 + annualReturnAssumption)^(1/12)` is rounded to 80 decimal places, with the mathematical monthly recurrence `balance = balance × q + contribution`. To prevent root approximation from shifting an exact annual threshold into the following month, powers are evaluated as `(1 + annualReturnAssumption)^floor(n/12) × q^(n mod 12)`: every complete year uses the supplied annual factor directly, not the rounded `q^12`. The contribution annuity sums these same powers. No comparison tolerance forgives real funding gaps, including a single 18-decimal unit. Scenario `requiredContribution` solves `(target − current × q^n) / (1 + q + … + q^(n−1))`, floored at zero. It is the **total** recurring contribution required, not the extra contribution above the existing plan. Already-completed goals require zero new contribution. With no remaining periods (today, past deadline, or before the first scheduled date), required values report the immediate remaining funding gap and `requiredContributionTiming` is `immediate`; the target-date projection is current value, not reconstructed historical wealth.
