...
const facts = () => ({
  ...
  wasmTarget: true,
  docker: false,
  git: { branch: "feat/m3-readiness", commit: "a".repeat(40), dirty: false },
  filesMissing: [],
  pinsAgree: true,
  network: {
    network: "testnet",
...
describe("developer environment diagnosis", () => {
  ...
  test.each([
    ["node", "22.23.1"],
    ["pnpm", "10.0.0"],
  ])("%s mismatch is actionable ERROR", (key, value) => {
    const f = facts();
    f.actual[key] = value;
    const r = evaluateDoctor(f);
    expect(r.exitCode).toBe(1);
    expect(r.checks.find((c) => c.id === key)).toMatchObject({
      level: "ERROR",
  ...
  test("optional Rust and wasm absence does not block web setup", () => {
    ...
    expect(
      r.checks
        .filter((c) => ["rust", "wasm"].includes(c.id))
        .every((c) => c.level === "WARNING"),
    ).toBe(true);
  });
  test("conflicting pins and missing required source fail clearly", () => {
    const f = facts();
    f.pinsAgree = false;
    f.filesMissing = ["pnpm-lock.yaml"];
    const r = evaluateDoctor(f);
    expect(r.exitCode).toBe(1);
    expect(r.checks.find((c) => c.id === "files").detail).toContain(
      "pnpm-lock.yaml",
    );
  ...
  test("dirty Git state is a warning without exposing file contents", () => {
    const f = facts();
    f.git.dirty = true;
    expect(evaluateDoctor(f).checks.find((c) => c.id === "git").level).toBe(
      "WARNING",
    );
  });
  test.each([
    null,
    { network: "mainnet" },
    { ...facts().network, nativeAsset: { baseDenom: "uzig", decimals: 6 } },
  ])("invalid or obsolete network configuration is an ERROR", (network) => {
    const f = facts();
    f.network = network;
    expect(evaluateDoctor(f).checks.find((c) => c.id === "network").level).toBe(
      "ERROR",
    );
  });
  test("failed command diagnostics suppress arbitrary stderr and use no shell", () => {
    let options;
    const result = probeCommand("not-a-tool", [], {
      runner: (_cmd, _args, opts) => {
        options = opts;
        return { status: 1, stdout: "", stderr: "private fixture value" };
      },
    });
