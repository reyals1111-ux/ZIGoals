...
test('real named Worker binding marks before fixture HTTP, coalesces, preserves partial results and survives restart',async()=>{
 ...
 const config={policy,month:{id:'fixture-month',start:now-1000,end:now+300000},quoteCost:3,operationCosts:{token:6},leaseMs:20000,maxAttempts:16,maxWorks:16};let mf,calls=0,release,observed;const sent=new Promise(r=>{observed=r;});const hold=new Promise(r=>{release=r;});
 ...
 mf=await runtime();try{
  ...
  const result=await owner;expect(result.results.map(r=>r.failure)).toEqual([null,'UPSTREAM_5XX']);expect(result.quotes[0].price).toBe('123456789123456789');expect(await inspect()).toMatchObject({chargedCredits:12,dispatched:0});
  expect((await load([{marketRef:{provider:'coingecko',kind:'rwa',id:'gold',assetType:'commodity'},currency:'EUR'}])).results[0].failure).toBe('UNSUPPORTED');expect(calls).toBe(3);
  await mf.dispose();mf=await runtime();expect((await load([pair('bitcoin')])).results[0].status).toBe('VERIFIED_FRESH');expect(calls).toBe(3);expect(await inspect()).toMatchObject({chargedCredits:12});
