...
test('real Workers coordinator persists dispatch, public cache, fences and replay tombstones across restart',async()=>{
 const persist=await mkdtemp(join(tmpdir(),'run10-market-'));const built=await build({entryPoints:[new URL('../../workers/market-coordinator/worker.ts',import.meta.url).pathname],bundle:true,write:false,format:'esm',platform:'browser',target:'es2022',external:['cloudflare:workers']});
