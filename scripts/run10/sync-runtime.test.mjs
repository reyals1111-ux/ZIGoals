...
test('real Workers durable storage: two clients, auth denial, conflict, retry, deletion and restart',async()=>{
 ...
 let mf=await runtime();const call=(who,body)=>mf.dispatchFetch('https://sync.test/v1/vault',{method:body?'POST':'GET',headers:{authorization:`Bearer ${who}`,origin:'https://app.test','content-type':'application/json'},...(body?{body:JSON.stringify(body)}:{})});
 try{
  ...
  const create={protocol:1,operation:crypto.randomUUID(),base:0,manifest:vault.manifest,changes:[]};
  ...
  const edit={protocol:1,operation:crypto.randomUUID(),base:1,changes:[record]};
  ...
  const del={protocol:1,operation:crypto.randomUUID(),base:2,changes:[{...record,revision:2,deleted:true}]};expect((await call('account-a',del)).status).toBe(200);
