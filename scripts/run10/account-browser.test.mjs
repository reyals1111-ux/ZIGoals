...
test.skipIf(process.env.RUN10_BROWSER!=='1')('two real browser profiles use encrypted account transport, local durable stores and persistent Worker; auth delivery is a fixture',async()=>{
 ...
 const mf=new Miniflare({...convertV4MiniflareOptions({name:'browser-private-sync',modules:true,script:await readFile(new URL('../../workers/private-sync/worker.mjs',import.meta.url),'utf8'),compatibilityDate:'2026-09-13',durableObjects:{VAULTS:{className:'PrivateVault',useSQLite:true}},durableObjectsPersist:persist,bindings:{AUTH_ORIGIN:'https://fixture.supabase.co',AUTH_PUBLIC_KEY:'public-fixture',APP_ORIGIN:origin},outboundService:async()=>Response.json({id:account})}),resourcePersistencePath:persist});
 const browser=await chromium.launch({channel:'chrome',headless:true});
 async function context(mobile=false){const c=await browser.newContext({viewport:mobile?{width:390,height:844}:{width:1280,height:900},isMobile:mobile});
  await c.route('**/api/private-account*',async route=>{const r=route.request(),headers=await r.allHeaders();const req=new Request(r.url(),{method:r.method(),headers,...(r.postData()?{body:r.postData()}: {})});
   const result=await privateAccountRequest(req,{authOrigin:'https://fixture.supabase.co',publicKey:'public-fixture',syncOrigin:'https://fixture.workers.dev'},async(url,init)=>{
    ...
    if(path.endsWith('/verify'))return Response.json({access_token:'fixture-token',expires_in:3600,user:{id:account}});
 ...
 try{
  ...
  const raw=await (await mf.dispatchFetch('https://fixture.workers.dev/v1/vault',{headers:{origin,authorization:'Bearer fixture-token','x-zigoals-account':account}})).text();expect(raw).not.toContain('Fictional synchronized reading');expect(JSON.parse(raw).records.length).toBeGreaterThan(1);
  await pa.reload();expect(await pa.getByRole('article',{name:'Fictional synchronized reading',exact:true}).count()).toBe(0);
